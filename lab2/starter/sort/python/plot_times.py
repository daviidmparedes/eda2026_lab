...
Por defecto lee times_random.csv (generado por ./test) y guarda el png junto al csv.
Para el otro barrido:
...
def main():
    csv_path = sys.argv[1] if len(sys.argv) > 1 else "times_random.csv"
    stem = os.path.splitext(os.path.basename(csv_path))[0]
    default_out = os.path.join(os.path.dirname(csv_path) or ".", stem + ".png")
